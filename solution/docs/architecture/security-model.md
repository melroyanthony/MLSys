...
| Path traversal | Low risk | CLI accepts file paths; use `pathlib.Path.resolve()` for safety |

## Input Validation

- All JSON parsing uses Python's standard `json` module (no `eval`, no `yaml.unsafe_load`)
